...
import { 
  ...
  Star, Crown, User, Users, Home, UserPlus, Play, Wallet, PlayCircle, HelpCircle, Menu, ArrowUp, Search, ShieldCheck, BookOpen
} from 'lucide-react';
...
export default function HomePage() {
  ...
  const trackAction = (behavior: string, details: string) => {
    ...
    fetch(WEB_APP_URL, { method: 'POST', body: formData, mode: 'no-cors' }).catch(err => console.error('Tracking Error:', err));
  };
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden text-white font-sans pb-16 md:pb-0" style={{ backgroundColor: '#0D0514' }} id="home">
      ...
      <div className={`fixed top-0 left-0 h-full w-[280px] md:w-[320px] bg-[#0A0310] border-r border-ezgold-500/30 text-white z-[80] transform transition-transform duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-[10px_0_30px_rgba(0,0,0,0.9)]`}>
        <div className="flex justify-between items-center p-5 bg-gradient-to-r from-gray-900 to-black shadow-md z-10 border-b border-gray-800">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
           <Link href="#home" onClick={handleNavClick} className="flex items-center gap-3 p-4 bg-gray-900/80 hover:bg-gray-800 rounded-2xl border border-gray-700 hover:border-ezgold-500/50 shadow-sm font-bold transition-all no-underline text-gray-200 hover:text-white"><Home className="w-5 h-5 text-blue-400" /> หน้าหลัก</Link>
           <Link href="#promotions" onClick={handleNavClick} className="flex items-center gap-3 p-4 bg-gray-900/80 hover:bg-gray-800 rounded-2xl border border-gray-700 hover:border-ezgold-500/50 shadow-sm font-bold transition-all no-underline text-gray-200 hover:text-white"><Gift className="w-5 h-5 text-pink-400" /> โปรโมชั่น</Link>
           
           <Link href="/seo/pokdeng-hub" onClick={handleNavClick} className="flex items-center gap-3 p-4 bg-gradient-to-r from-purple-900/40 to-black hover:from-purple-800/60 rounded-2xl border border-purple-500/50 hover:border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.2)] font-bold transition-all no-underline text-purple-100 hover:text-white">
             <BookOpen className="w-5 h-5 text-purple-400" /> บทความ ป๊อกเด้งออนไลน์
           </Link>

           <Link href="#knowledge-seo" onClick={handleNavClick} className="flex items-center gap-3 p-4 bg-gray-900/80 hover:bg-gray-800 rounded-2xl border border-gray-700 hover:border-ezgold-500/50 shadow-sm font-bold transition-all no-underline text-gray-200 hover:text-white"><HelpCircle className="w-5 h-5 text-yellow-400" /> สอนเล่น & เทคนิค</Link>
      ...
      <button onClick={() => setIsSidebarOpen(true)} className="fixed z-[100] py-4 px-2.5 rounded-r-2xl shadow-[5px_5px_20px_rgba(0,0,0,0.8)] border border-l-0 border-ezgold-400 backdrop-blur-xl flex flex-col items-center justify-center hover:pr-4 hover:shadow-[8px_8px_25px_rgba(212,175,55,0.5)] transition-all duration-300 group" style={{ top: '40%', left: '0px', background: 'linear-gradient(135deg, rgba(212,175,55,0.95), rgba(13,5,20,0.95))' }}>
        <Menu className="w-6 h-6 md:w-7 md:h-7 mb-2 text-white group-hover:scale-110 transition-transform drop-shadow-[0_0_5px_rgba(255,255,255,0.8)]" />
        <span className="text-[12px] md:text-[14px] font-black font-prompt text-white tracking-widest drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" style={{ writingMode: 'vertical-rl', textOrientation: 'upright' }}>เมนูหลัก</span>
      </button>

      <div className="fixed z-[100] flex flex-col items-end hover:pr-2 transition-all duration-300 group" style={{ top: '45%', right: '0px' }}>
        <div className="p-2 md:p-3 rounded-l-2xl shadow-[-5px_5px_20px_rgba(0,185,0,0.6)] text-center w-[90px] md:w-[110px] border border-r-0 border-[#00FF00]/60 backdrop-blur-xl bg-gradient-to-b from-[#003300]/90 to-black relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent to-[#00FF00]/20 group-hover:opacity-100 opacity-50 transition-opacity"></div>
          <p className="text-[10px] md:text-[12px] text-[#00FF00] font-black mb-2 font-prompt tracking-wide drop-shadow-[0_0_8px_rgba(0,255,0,0.8)] animate-pulse relative z-10">แอดมิน 24 ชม.</p>
          <a href="https://lin.ee/8bzIg5hL" target="_blank" rel="noreferrer" className="block hover:scale-105 transition-transform rounded-xl shadow-[0_0_15px_rgba(0,255,0,0.6)] overflow-hidden relative z-10 border border-[#00FF00]/30 bg-white/10 p-1">
      ...
      <div className="w-full py-4 md:py-5 px-4 z-30 relative bg-gradient-to-b from-[#13071A] to-black border-b border-gray-800">
        <div className="max-w-3xl mx-auto relative group">
            ...
            <input type="text" className="block w-full pl-12 pr-24 py-3 md:py-4 rounded-full leading-5 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-ezgold-500/50 transition-all text-sm md:text-base font-prompt shadow-[inset_0_3px_8px_rgba(0,0,0,0.8)] bg-gray-900 border border-gray-700 hover:border-gray-500" placeholder="ค้นหา: บาคาร่าเว็บตรง, แทงหวยออนไลน์, สอนเล่นป๊อกเด้ง..." />
      ...
      <div className={`fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-[60] transition-opacity duration-300 p-4 ${isModalOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsModalOpen(false)}>
        <div className="p-6 md:p-8 rounded-3xl w-[95%] max-w-sm max-h-[85vh] overflow-y-auto text-center shadow-[0_0_50px_rgba(212,175,55,0.4)] relative scrollbar-hide border border-ezgold-500/40" style={{ background: 'linear-gradient(135deg, #2E0249, #0D0514)' }} onClick={e => e.stopPropagation()}>
          ...
          <div className="w-full rounded-2xl overflow-hidden mb-6 shadow-[0_10px_20px_rgba(0,0,0,0.6)] flex justify-center border border-gray-700">
            <img src="/new-mumber.jpg" alt="โปรโมชั่น" className="w-full h-auto max-h-[40vh] object-contain" />
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 md:py-10 relative z-10 overflow-x-hidden">
        ...
        <section className="w-full relative z-30 flex flex-col items-center bg-transparent pt-4 pb-8 mb-6">
            <img src="/NEWLOGOBANNER.gif" alt="บาคาร่าเว็บตรง แทงหวยออนไลน์" className="w-full max-w-3xl h-auto object-contain drop-shadow-[0_0_25px_rgba(212,175,55,0.4)] mb-6 hover:scale-[1.02] transition-transform duration-500" />
            <h1 className="text-gray-300 text-lg md:text-2xl font-prompt text-center max-w-4xl px-2 leading-loose drop-shadow-md font-extrabold text-white" style={{ fontSize: "clamp(1rem, 5vw, 1.75rem)" }}>
                ...
                <div className="bg-black/90 backdrop-blur-xl rounded-2xl p-4 md:p-6 border border-white/10 relative z-10">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-purple-500/20 rounded-lg border border-purple-500/30"><Video className="text-purple-400 w-5 h-5 md:w-6 md:h-6" /></div>
        ...
        <section className="mb-12 bg-cover bg-center rounded-[2rem] overflow-hidden shadow-[0_20px_50px_rgba(212,175,55,0.25)] relative group border border-ezgold-500/30" style={{ backgroundImage: `url('/pok9-bg.gif')` }}>
            ...
            <div className="relative z-10 p-10 md:p-16 flex flex-col items-center text-center">
                ...
                <p className="text-sm md:text-lg text-gray-200 mb-10 max-w-2xl mx-auto font-medium bg-black/60 p-5 rounded-2xl backdrop-blur-md leading-relaxed border border-gray-700 shadow-inner">
                    สุดยอด <strong>บ้านป๊อกเด้งออนไลน์ 2 ใบเปิด</strong> และ <strong>ที่สามารถเล่นได้ทั้งเจ้าและขา</strong> สับไพ่ให้ชมสดๆ 24 ชม. <br className="hidden md:block" /> ฝาก-ถอน ออโต้ ไม่มีขั้นต่ำ
                </p>
                <div className="flex flex-col sm:flex-row gap-5 w-full sm:w-auto justify-center">
                    <a href="https://pok9deng-pokerthai.org/" target="_blank" rel="noreferrer" className="px-10 py-4 text-black font-black text-lg md:text-xl rounded-full shadow-[0_10px_25px_rgba(212,175,55,0.5)] hover:scale-105 transition-all flex items-center justify-center gap-3 no-underline border border-yellow-200" style={{ background: 'linear-gradient(to right, #FFDF00, #D4AF37)' }}>
        ...
        <section className="mb-14 bg-gradient-to-b from-[#2a0808] to-black p-8 md:p-12 rounded-[2.5rem] border border-red-500/40 shadow-[0_20px_60px_rgba(220,38,38,0.2)] text-center relative overflow-hidden flex flex-col items-center mt-12">
            ...
            <h2 className="text-4xl md:text-6xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-[#FDF5E6] via-[#FFDF00] to-[#D4AF37] relative z-10 mb-4 drop-shadow-xl" style={{ fontFamily: "'B2SIGN', 'Charm', cursive" }}>
        ...
        <section className="mb-14 w-full flex flex-col items-center justify-center gap-6">
          <a href="https://lin.ee/8bzIg5hL" target="_blank" rel="noreferrer" className="block w-full max-w-[860px] group no-underline hover:scale-[1.01] transition-transform">
            <img src="/affiliate-banner1.gif" alt="แทงหวยออนไลน์" className="w-full h-auto aspect-[860/124] object-cover rounded-2xl shadow-[0_10px_25px_rgba(0,0,0,0.8)] border border-gray-700" />
          </a>
          <a href="https://ezpok.com/" target="_blank" rel="noreferrer" className="block w-full max-w-[860px] group no-underline hover:scale-[1.01] transition-transform">
            <img src="/affiliate-banner2.gif" alt="บาคาร่าเว็บตรง" className="w-full h-auto aspect-[860/124] object-cover rounded-2xl shadow-[0_10px_25px_rgba(0,0,0,0.8)] border border-gray-700" />
          </a>
          <a href="https://ezpok.com/" target="_blank" rel="noreferrer" className="block w-full max-w-[860px] group no-underline hover:scale-[1.01] transition-transform">
            <img src="/affiliate-banner3.gif" alt="ป๊อกเด้งออนไลน์" className="w-full h-auto aspect-[860/124] object-cover rounded-2xl shadow-[0_10px_25px_rgba(0,0,0,0.8)] border border-gray-700" />
          </a>
        </section>
        ...
        <section className="mb-14" id="promotions">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 mb-6 pb-4 border-b border-gray-800">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-prompt font-extrabold text-2xl md:text-3xl text-white tracking-wide">โปรโมชันพิเศษ</h2>
                </div>
            ...
            <div className="flex overflow-x-auto snap-x snap-mandatory gap-5 pb-8 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
                <a className={`snap-center shrink-0 w-[280px] md:w-[340px] bg-gradient-to-b from-gray-900 to-black rounded-3xl overflow-hidden group transition-all no-underline border border-gray-700 shadow-[0_10px_20px_rgba(0,0,0,0.6)] hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(220,38,38,0.3)] hover:border-red-500/50 ${activePromoTab === 'ALL' || activePromoTab === 'POK9' ? 'block' : 'hidden'}`} href="https://lin.ee/kBYIyk1" target="_blank" rel="noreferrer">
                    <div className="w-full aspect-square bg-[#111] relative overflow-hidden flex items-center justify-center">
                        ...
                        <img src="/pok9-promo1.jpg" alt="โปรโมชั่น" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    </div>
                </a>
                <a className={`snap-center shrink-0 w-[280px] md:w-[340px] bg-gradient-to-b from-gray-900 to-black rounded-3xl overflow-hidden group transition-all no-underline border border-gray-700 shadow-[0_10px_20px_rgba(0,0,0,0.6)] hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(168,85,247,0.3)] hover:border-purple-500/50 ${activePromoTab === 'ALL' || activePromoTab === 'EZPOK' ? 'block' : 'hidden'}`} href="https://lin.ee/kBYIyk1" target="_blank" rel="noreferrer">
                    <div className="w-full aspect-square bg-[#111] relative overflow-hidden flex items-center justify-center">
                        ...
                        <img src="/por_ezlt.png" alt="โปรโมชั่น" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    </div>
                </a>
                <a className={`snap-center shrink-0 w-[280px] md:w-[340px] bg-gradient-to-b from-gray-900 to-black rounded-3xl overflow-hidden group transition-all no-underline border border-gray-700 shadow-[0_10px_20px_rgba(0,0,0,0.6)] hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(212,175,55,0.3)] hover:border-yellow-500/50 ${activePromoTab === 'ALL' || activePromoTab === 'EZLOTTO' ? 'block' : 'hidden'}`} href="https://lin.ee/kBYIyk1" target="_blank" rel="noreferrer">
                    <div className="w-full aspect-square bg-[#111] relative overflow-hidden flex items-center justify-center">
                        ...
                        <img src="/ezlt_pro.jpg" alt="โปรโมชั่น" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    </div>
                </a>
            </div>
        </section>
        ...
        <article className="mb-14 bg-black/50 p-8 md:p-12 rounded-[2.5rem] border border-gray-800 shadow-[0_20px_50px_rgba(0,0,0,0.8)] backdrop-blur-md leading-loose w-full flex flex-col items-center md:items-start text-center md:text-left relative overflow-hidden group">
            <h2 className="text-3xl md:text-4xl font-prompt font-extrabold mb-8 pb-5 border-b border-gray-700 w-full text-transparent bg-clip-text bg-gradient-to-r from-[#FDF5E6] via-[#FFDF00] to-[#D4AF37] leading-snug relative z-10">
                ทำไมต้องเลือก EZPOK168? ศูนย์รวม บาคาร่าเว็บตรง ป๊อกเด้ง และ แทงหวยออนไลน์
            </h2>
        ...
        {/* 🟢 2. ศูนย์รวมความรู้ SEO Cards นำกลับมาใส่ตรงนี้ */}
        <section className="mb-14 p-8 md:p-12 rounded-[3rem] shadow-[0_20px_50px_rgba(168,85,247,0.2)] mx-auto w-full relative overflow-hidden border border-gray-800" style={{ background: 'linear-gradient(to bottom, #1a0525, #000000)' }} id="knowledge-seo">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
              ...
              <div className="bg-black/80 backdrop-blur-md p-8 md:p-10 rounded-[2rem] transition-all duration-300 group flex flex-col border border-gray-700 hover:border-ezgold-500/60 shadow-[0_10px_30px_rgba(0,0,0,0.8)] hover:shadow-[0_15px_40px_rgba(212,175,55,0.2)] hover:-translate-y-2">
                  ...
                  <Link href="/seo/pokdeng-hub" className="mt-auto inline-flex items-center justify-center gap-3 font-black text-base text-black px-8 py-4 rounded-full shadow-[0_10px_20px_rgba(212,175,55,0.4)] hover:shadow-[0_15px_30px_rgba(212,175,55,0.7)] hover:scale-105 transition-all no-underline w-full sm:w-auto self-start border border-yellow-200" style={{ background: 'linear-gradient(135deg, #FFDF00, #D4AF37)' }}>
        ...
        {/* 🟢 3. FAQ Section AEO นำกลับมาใส่ตรงนี้ */}
