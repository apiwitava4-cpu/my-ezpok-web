...
const laoLottoData = {
  "10042569": { prize4: "4329", prize3: "329", top2: "29", bot2: "43" },
  "08042569": { prize4: "7704", prize3: "704", top2: "04", bot2: "77" },
  "06042569": { prize4: "1952", prize3: "952", top2: "52", bot2: "19" }
};

export default function LaoLottoChecker() {
  const [date, setDate] = useState("10042569");
    ...
    <>
      <form onSubmit={handleCheck} className="w-full max-w-2xl mx-auto flex flex-col gap-5 relative z-10 mt-6">
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full bg-black/40 p-4 rounded-xl border border-gray-800">
          <label htmlFor="lao_issue_date" className="text-ezgold-400 font-bold whitespace-nowrap font-prompt">เลือกงวดวันที่:</label>
          <select 
            id="lao_issue_date" 
            value={date}
          ...
          >
            <option value="10042569">10 เมษายน 2569</option>
            <option value="08042569">8 เมษายน 2569</option>
            <option value="06042569">6 เมษายน 2569</option>
          </select>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
          {inputs.map((val, idx) => (
            <input 
              ...
              className="w-full px-4 py-3 bg-[#111] border border-gray-700 rounded-xl focus:border-ezgold-500 focus:ring-1 focus:ring-ezgold-500 focus:outline-none text-white font-prompt tracking-[0.3em] text-center text-lg placeholder:tracking-normal placeholder:text-sm placeholder:text-gray-600 transition-all" 
              placeholder={`กรอกเลข 2-4 ตัว`} 
              autoComplete="off" 
            />
          ))}
        </div>

        <button type="submit" className="mt-3 w-full sm:w-auto mx-auto px-10 py-4 bg-gradient-to-r from-purple-600 to-purple-400 text-white font-extrabold text-lg md:text-xl rounded-xl hover:scale-105 transition-transform shadow-[0_0_15px_rgba(168,85,247,0.4)] flex items-center justify-center gap-3 font-prompt">
          <Search className="w-6 h-6" /> ตรวจหวยลาวพัฒนา
        </button>
      </form>
      ...
      {modal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-purple-500 rounded-2xl p-6 md:p-8 max-w-md w-full text-center relative shadow-[0_0_30px_rgba(168,85,247,0.3)] animate-in fade-in zoom-in duration-300">
            ...
            <button type="button" onClick={() => setModal({...modal, isOpen: false})} className="w-full px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-400 text-white font-bold rounded-xl hover:scale-105 transition-transform text-lg font-prompt">
              ตกลง
            </button>
          </div>
        </div>
